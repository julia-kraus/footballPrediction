...
class multivariaLinearRegression():
    ...
    def constractFeatures(self):
        ...
        self.features = [self.pastResult.getFeature(x[2], x[3], x[1], x[6], K) for x in self.data]
        self.features2 = [self.pastResult2.getFeature(x[2], x[3], x[1], x[6], K) for x in self.data2]
        self.features3 = [self.pastResult3.getFeature(x[2], x[3], x[1], x[6], K) for x in self.data3]
        self.testFeatures = [self.pastTestResult.getFeature(x[2], x[3], x[1], x[6], K) for x in self.testData]

    def doLF(self):
        x = [x[1:2 * K + 1] for x in self.features]
        y = [y[0] for y in self.features]

        x1 = [x[1:2 * K + 1] for x in self.features2]
        y1 = [y[0] for y in self.features2]

        x3 = [x[1:2 * K + 1] for x in self.features3]
        y3 = [y[0] for y in self.features3]

        x2 = [x[1:2 * K + 1] for x in self.testFeatures]
        y2 = [y[0] for y in self.testFeatures]
